...
const HaushaltReviewContainer: React.FC<HaushaltReviewContainerProps> = ({ residentId }) => {
  ...
  const validateForm = () => {
    ...
    if (!String(formData.mainApplicant.birthDate || '').trim()) {
      ...
      errors.mainApplicant.push('Geburtsdatum liegt außerhalb des gültigen Bereichs (nicht mehr als 115 Jahre in der Vergangenheit oder mehr als 10 Monate in der Zukunft)');
    }
    ...
    if (!String(formData.mainApplicant.entrydate || '').trim()) {
      ...
      errors.mainApplicant.push('Aufnahme in den Haushalt liegt außerhalb des gültigen Bereichs (nicht mehr als 115 Jahre in der Vergangenheit oder mehr als 3 Jahre in der Zukunft)');
    }
    if (!String(formData.mainApplicant.behinderungsgrad || '').trim()) {
      ...
      errors.mainApplicant.push('Grad der Behinderung muss einer der folgenden Werte sein: 0, 20, 30, 40, 50, 60, 70, 80, 90, 100');
    }
    if (!String(formData.mainApplicant.pflegegrad || '').trim()) {
      ...
      errors.mainApplicant.push('Pflegegrad muss eine ganze Zahl zwischen 0 und 5 sein');
    }
